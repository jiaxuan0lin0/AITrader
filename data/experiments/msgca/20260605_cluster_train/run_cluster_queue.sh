...
CODE=code
PY=python3
CTX=data/datasets/features/blocks/sample/context_theme_cluster_sample.parquet
...
path = Path("data/datasets/features/blocks/sample/context_theme_cluster_sample.parquet")
...
wait_context_cache() {
  until context_cache_ready; do
    log "waiting context cache: $CTX"
    sleep 60
  done
  log "cluster context cache ready: $CTX"
}
...
wait_context_cache
for run_id in cluster_ctx020_scratch_seed2031 cluster_inrank020_scratch_seed2031; do
  train_run "$run_id"
  eval_run "$run_id"
